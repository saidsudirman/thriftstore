...
@section('sidebar_subtitle', 'Edit Kategori')
@section('page_icon')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-soft border border-gray-100 overflow-hidden">
        <div class="border-b border-gray-100 px-6 py-4 bg-gradient-to-r from-amber-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-semibold text-gray-800">Form Edit Kategori</h2>
                    <p class="text-xs text-gray-500">Ubah data kategori yang diperlukan</p>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-transparent transition">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div class="bg-blue-50 rounded-xl p-4 border border-blue-100">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h4 class="font-semibold text-gray-800 text-sm">Informasi Kategori</h4>
                        <p class="text-gray-600 text-xs mt-1">
                            ID Kategori: <span class="font-mono font-semibold">{{ $category->id }}</span><br>
                            Dibuat pada: {{ $category->created_at ? $category->created_at->format('d M Y H:i') : '-' }}<br>
                            Terakhir update: {{ $category->updated_at ? $category->updated_at->format('d M Y H:i') : '-' }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-4">
        ...
        <p class="text-gray-400 text-xs mt-2">* Menghapus kategori tidak akan menghapus produk, hanya akan menghapus pengelompokan kategori pada produk tersebut.</p>
    </div>
</div>
@endsection